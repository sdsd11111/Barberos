...
const DB_NAME = "BarberOS_Music_DB";
...
function openDB(): Promise<IDBDatabase> {
  return new Promise((resolve, reject) => {
    if (typeof window === "undefined" || !window.indexedDB) {
      reject("IndexedDB no está disponible en este entorno");
      return;
    }
...
export async function getLocalTracksFromDB(): Promise<StoredTrack[]> {
  try {
    ...
    console.error("Error al obtener canciones de IndexedDB:", err);
    return [];
  }
}
...
export async function clearAllLocalTracksFromDB(): Promise<void> {
  const db = await openDB();
  const tx = db.transaction(STORE_NAME, "readwrite");
  const store = tx.objectStore(STORE_NAME);
  store.clear();
